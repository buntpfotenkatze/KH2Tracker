 ...
 */
internal class CheckEveryCheck
{
    ...
    private bool onLevels;
    private Dictionary<string, int> targetLevel;
    private bool onValor;
    ...
    private readonly List<Check> checksList;
    private List<Check> targetCheck; // list in case there are duplicates
    private bool targetObtained;
    public bool TargetObtained => targetObtained;

    public CheckEveryCheck(
        MemoryReader mem,
        int offset,
        int saveAnchor,
        int sysAnchor,
    ...
    private void FindTargetCheck()
    {
        ...
        {
            var checkValueBytes = Memory.ReadMemory(check.AddrCheck + AddressOffset, 2);
        ...
        if (!MainWindow.Data.Codes.ItemCodes.TryGetValue(target, out var itemCode))
            return;
        ...
        var shieldLevel = 100;
        var staffLevel = 100;
        var swordReward = Rewards
            .GetLevelRewards("Sword")
            .FirstOrDefault(reward => reward.Item2 == itemCode);
        var shieldReward = Rewards
            .GetLevelRewards("Shield")
            .FirstOrDefault(reward => reward.Item2 == itemCode);
        var staffReward = Rewards
            .GetLevelRewards("Staff")
            .FirstOrDefault(reward => reward.Item2 == itemCode);
        if (swordReward != null)
        {
        ...
        // Drive levels
        var valorReward = Rewards.ValorChecks.FirstOrDefault(reward => reward.Item2 == itemCode);
        var wisdomReward = Rewards.WisdomChecks.FirstOrDefault(reward => reward.Item2 == itemCode);
        var limitReward = Rewards.LimitChecks.FirstOrDefault(reward => reward.Item2 == itemCode);
        var masterReward = Rewards.MasterChecks.FirstOrDefault(reward => reward.Item2 == itemCode);
        var finalReward = Rewards.FinalChecks.FirstOrDefault(reward => reward.Item2 == itemCode);
        if (
            valorReward == null
            && wisdomReward == null
            && limitReward == null
    ...
    public bool UpdateTargetMemory()
    {
        ...
        {
            foreach (var check in targetCheck)
            {
    ...
    private class Check
    {
        public readonly CheckEveryCheck Outer;

        public readonly int AddrCheck;
        public readonly int AddrObtained;
        public readonly int Bit;
        public const int Bytes = 1;
        private readonly bool asdata;
            ...
            if (asdata) // assumes World is updated
            {
                if (!obtained)
                {
                    obtained =
                        ...
                        && World.RoomNumber == RoomId
                        && World.EventId3 == EventId;
                }
            }
            else
            {
                var data = Outer.Memory.ReadMemory(AddrObtained + Outer.AddressOffset, Bytes);
